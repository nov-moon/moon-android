...
public class NumberPicker extends View {
    ...
    private TextPaint mTextPaintForUnSelect;
    ...
    private int mTextColorForUnSelect;
    ...
    private void readAttrs(Context context, AttributeSet attrs, int defStyleAttr) {
        final TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.NumberPicker, defStyleAttr, R.style.defNumberPickerStyle);

        mTextColor = a.getColor(R.styleable.NumberPicker_textColor, DEFAULT_TEXT_COLOR);
        mTextColorForUnSelect = a.getColor(R.styleable.NumberPicker_textColorUnSelect, DEFAULT_TEXT_COLOR);
        mTextSize = a.getDimension(R.styleable.NumberPicker_textSize, DEFAULT_TEXT_SIZE * mDensity);
        mTextSizeUnSelect = a.getDimension(R.styleable.NumberPicker_textSizeUnSelect, DEFAULT_TEXT_SIZE * mDensity);
    ...
    private void initPaint() {
        ...
        mTextPaintForUnSelect = new TextPaint(mTextPaint);
        mTextPaintForUnSelect.setColor(mTextColorForUnSelect);
        mTextPaintForUnSelect.setTextSize(mTextSizeUnSelect);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // Draw numbers
        for (NumberHolder numberHolder : mTextYAxisArray) {
            if (numberHolder.mmIndex >= 0 && numberHolder.mmIndex <= mEndNumber - mStartNumber) {
                String text;
                if (mTextArray != null) {
                    text = mTextArray[numberHolder.mmIndex];
                } else {
                    text = String.valueOf(mNumberArray[numberHolder.mmIndex]);
                }
                if (mCurrNumIndex == numberHolder.mmIndex) {
                    canvas.drawText(
                            text,
                            mWidth / 2,
                            numberHolder.mmPos + mTextBounds.height() / 2,
                            mTextPaint);
                } else {
                    canvas.drawText(
                            text,
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
         ...
         */
        if (null == mHighLightRect || old > mHeight) {
    ...
    public void set(int startNumber, int endNumber, int currentNumber) {
