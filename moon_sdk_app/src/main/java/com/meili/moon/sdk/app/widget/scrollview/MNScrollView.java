package com.meili.moon.sdk.app.widget.scrollview;
...
/**
 * Created by zhengshuai on 2018/5/18.
 */
public class MNScrollView extends FrameLayout implements OnTouchListener {
	...
	private  int childPaddingTop = 0;
	private  int childPaddingBottom = 0;

	private static int overYDistance = 0 ;
	...
	public MNScrollView(Context context, AttributeSet attrs, int defStyle) {
	...
	public void initChildPointer() {
		...
		if(mContext!=null){
			...
			overYDistance = wm.getDefaultDisplay().getHeight();
		}
		childPaddingTop = child.getPaddingTop();
		childPaddingBottom = child.getPaddingBottom();
		child.setPadding(child.getPaddingLeft(), child.getPaddingTop()+overYDistance, child.getPaddingRight(), child.getPaddingBottom()+overYDistance);
	...
	public boolean isOverScrolled() {
		return (getScrollY() < overYDistance || getScrollY() > child.getBottom() - overYDistance - getHeight());
	}
	...
	public final void smoothScrollToTop() {
		smoothScrollTo(0, overYDistance);
	}

	public final void smoothScrollToBottom() {
		smoothScrollTo(0, child.getHeight() - overYDistance - getHeight());
	}
	...
	private boolean overScrollView() {
		...
		if (currScrollY < contentTop) {
			...
			// Scroll to content top
			if (child.getHeight() - overYDistance - child.getPaddingBottom() < displayHeight) {
